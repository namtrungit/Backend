...
module.exports = {
    add_area: function (req, res) {
        ...
        Areas.findOne({ area_sympol }).exec(function (err, find) {
            if (err) {
                console.log(err);
                return;
            }
            if (find) {
                res.json({
                    status: 'warning',
                    message: 'Đã có khu với ký hiệu ' + area_sympol + ', vui lòng hãy chọn ký hiệu khu khác'
                })
                return;
            }
            else {
                Areas.create({ area_address, area_sympol }).exec(function (err, created) {
                    if (err) {
                        console.log(err);
                        return;
                    }
    ...
    del_area: function (req, res) {
        ...
        Areas.findOne({ area_id }).exec(function (err, find) {
            ...
            if (find) {
                Rooms.findOne({ room_id_area: area_id }).exec(function (err, find) {
                    if (err) {
                        console.log(err);
                        return;
                    }
                    if (find) {
                        res.json({
                            status: 'warning',
                            message: 'Vui lòng xóa hết phòng trong khu này để thực hiện chức năng này!'
                        })
                        return;
                    }
                    else {
    ...
    update_area: function (req, res) {
        var area_id = req.param('area_id'),
            ...
            update_area_sympol = req.param('update_area_sympol');
        if (!area_id || area_id === '' || area_id < 1) {
            res.json({
                status: 'error',
                message: 'area_id khu vực không hợp lệ!'
        ...
        if (update_area_sympol) {
            console.log('có nhập ký hiệu khu');
            Areas.findOne({  area_sympol:update_area_sympol }).exec(function (err, find) {
                if (err) {
                    console.log(err);
                    return;
                }
                if (find) {
                    res.json({
                        status: 'warning',
                        message: 'Đã có khu với ký hiệu ' + update_area_sympol + ', vui lòng hãy chọn ký hiệu khu khác'
                    })
                    return;
                }
                else {
                    Areas.findOne({ area_id }).exec(function (err, find) {
                        ...
                        if (find) {
                            Areas.update({ area_id }, { area_address, area_sympol: update_area_sympol }).exec(function (err, updated) {
                                if (err) {
                                    console.log(err);
                                    return;
                                }
            ...
            console.log('không nhập ký hiệu khu');
            Areas.findOne({ area_id }).exec(function (err, find) {
                if (err) {
                    console.log(err);
                    return;
                ...
                if (find) {
                    Areas.update({ area_id }, { area_address }).exec(function (err, updated) {
                        if (err) {
                            console.log(err);
                            return;
                        }
